"""Generate the DISPLAY model (docs/urdf_Elrobot_viz.urdf) from the real URDF.

The real URDF (docs/urdf_Elrobot.urdf) is what IK and the driver use; this
script derives a renderer-friendly copy for rviz2 and Foxglove. Steps:

1. Joint names with spaces ("Rigid 1") become "Rigid_1" - some parsers choke.
2. The jaw link FRAMES are moved onto the jaws (the vendor put them ~38 cm
   away in CAD-world coordinates) without moving a single rendered pixel.
3. A camera mount + REP-103 optical frame are added (fixed joints; the mount
   position is the CAM_MOUNT_XYZ calibration knob below).
4. Every mesh is rewritten from the vendor's CAD-world-millimeter STL into a
   link-local COLLADA (.dae) in meters with Z_UP and a material color baked
   in - the one conventional format that rviz AND Foxglove render the same.
   Rebased meshes land in data/viz_meshes/ (derived, gitignored).

Meshes are vendored from https://github.com/norma-core/norma-core (MIT),
see docs/assets/LICENSE.norma-core.

Output is regenerated by every launch task; never edit it by hand.
"""
...
def fix_jaw_frames(root):
    ...
    for j in root.findall("joint"):
        ...
        delta = old - new  # what the frame moved by; the visual moves back
        el = link.find("visual/origin")
