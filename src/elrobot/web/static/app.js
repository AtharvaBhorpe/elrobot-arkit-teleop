...
const modeButtons = [...document.querySelectorAll(".dof-switch [data-mode]")];
...
function connect() {
	...
	ws.onmessage = (e) => {
		...
		if (Date.now() < bannerHoldUntil) {
			...
			banner.textContent =
				`Monitor only — another cockpit tab has control ` +
			...
			banner.textContent =
				"Another commander is also live (phone/jog) — web " +
		...
		setPill(
			"driver",
			m.driver_alive,
			m.driver_alive ? "driver live" : "driver down",
		);
		setPill(
			"age",
			m.age_s < 0.5,
			`state ${m.age_s < 9 ? (m.age_s * 1000) | 0 : "----"} ms`,
		);
		renderMode(m.mode);
		if (!document.body.hasAttribute("data-control"))
			// monitor: track
...
async function setPhoneMode(mode) {
	...
	if (!res.ok) {
		...
		flashBanner(r.detail || "teleop mode change was refused");
		return;
	}
	renderMode(mode);
}
...
async function setControl(on) {
	...
	for (const [n, v] of Object.entries(r.seed ?? {})) // no-jump seeding
		if (rows[n]) {
			rows[n].inp.value = v;
			rows[n].out.textContent = v.toFixed(2);
		}
}
...
for (const b of modeButtons)
	b.addEventListener("click", () => setPhoneMode(b.dataset.mode));
...
const ARM = Array.from(
	{ length: 7 },
	(_, i) => `rev_motor_${String(i + 1).padStart(2, "0")}`,
);
const calibEl = {
...
const CALIB_HINT = {
	...
	preflight:
		"Park the arm relaxed, no joint near a hard stop, then write EEPROM.",
	...
	fullturn:
		"Sweeping a full-turn joint - move it to both stops, then End sweep.",
...
function renderCalib(snap) {
	if (snap.error) {
		...
		calibEl.status.textContent = `${snap.state} - ${CALIB_HINT[snap.state] ?? ""}`;
	}
	...
	calibEl.backup.textContent = b
		? `backup: ${b}  (restore: pixi run calib-restore)`
		: "";
	...
	calibEl.gate.textContent = (snap.gate || [])
		.map(
			(g) =>
				`${g.name}  ${(g.span_pct * 100).toFixed(0)}%  ${g.ok ? "ok" : "SUSPECT"}`,
		)
		.join("\n");
...
calibEl.finish.addEventListener("click", () =>
	calibAction("/api/calib/finish"),
);
document
	.getElementById("calib-abort")
	.addEventListener("click", () => calibAction("/api/calib/abort")); // always available: frees the port
...
calibEl.eepromConfirm.addEventListener("click", async () => {
	try {
		const snap = await calibApi("/api/calib/eeprom", {
			confirm: calibEl.eepromInput.value,
		});
		renderCalib(snap);
...
async function refreshTasks() {
	...
	collectionEl.taskEdit.disabled =
		!collectionEl.task.value || collectionSnapshot.state === "recording";
}
...
function renderCollection(snapshot, recorder) {
	...
	collectionEl.sessionStart.disabled =
		snapshot.state !== "idle" || !collectionEl.task.value;
	...
	collectionEl.sessionFinish.disabled = !(ready || recording);
	...
	const frameText =
		recording && recorder ? ` · ${recorder.frames ?? 0} frames accepted` : "";
	collectionEl.status.textContent =
		snapshot.error ||
		(recording
...
async function collectionAction(path, body) {
	...
	try {
		...
		// A recorder process can have restarted between WebSocket updates.
		// Refresh before reporting the rejected click so controls match its
		// in-memory lifecycle state.
		try {
			await refreshCollection();
		} catch {
			// Preserve the action error if the recovery read also fails.
		}
...
collectionEl.sessionStart.addEventListener("click", () =>
	collectionAction("/api/collection/session/start", {
		task_id: collectionEl.task.value,
		name: collectionEl.sessionName.value,
	}),
);
collectionEl.episodeStart.addEventListener("click", () =>
	collectionAction("/api/collection/episode/start", {
		task_id: collectionEl.task.value,
	}),
);
collectionEl.episodeStop.addEventListener("click", () =>
	collectionAction("/api/collection/episode/stop"),
);
collectionEl.episodeDiscard.addEventListener("click", () =>
	collectionAction("/api/collection/episode/discard"),
);
...
collectionEl.task.addEventListener("change", () =>
	renderCollection(collectionSnapshot),
);
...
document
	.getElementById("task-create-open")
	.addEventListener("click", () => openTaskDialog());
collectionEl.taskEdit.addEventListener("click", () =>
	openTaskDialog(taskById(collectionEl.task.value)),
);
taskDialog.save.addEventListener("click", async () => {
	try {
		...
		else await jsonRequest("/api/tasks", "POST", body);
		...
		for (const session of sessions) {
			...
			for (const [text, action] of [
				...
				button.addEventListener("click", async () => {
					try {
						await jsonRequest(
							`/api/collection/recovery/${session.id}/${action}`,
							"POST",
						);
...
document
	.getElementById("recovery-refresh")
	.addEventListener("click", refreshRecoveries);
		...
		...tasks
			.map((task) => ({
				...
				episodes: curateEpisodes.filter(
					(episode) => effectiveTaskId(episode) === task.id,
				).length,
			}))
...
function renderEpisodeList() {
	const visible = curateEpisodes.filter(
		(episode) => !curateTaskId || effectiveTaskId(episode) === curateTaskId,
	);
	...
	for (const episode of visible) {
		...
		button.setAttribute(
			"aria-current",
			String(
				selectedEpisode && episodeKey(selectedEpisode) === episodeKey(episode),
			),
		);
		...
		details.textContent =
			`${episode.review} · ${episode.effective_frames} frames` +
...
async function refreshCurate() {
	try {
		...
		curateSessions = (await request("/api/curation/sessions")).sessions ?? [];
		const groups = await Promise.all(
			curateSessions.map((session) =>
...
function renderEpisodeEditor() {
	...
	const task = taskById(effectiveTaskId(episode));
	curateEl.title.textContent = `${episode.session_name || "Session"} · #${episode.source_index}`;
	...
	for (const [id, review] of [
		...
		document
			.getElementById(id)
			.classList.toggle("primary", episode.review === review);
	}
}
...
async function showFrame(n) {
	...
	rEl.status.textContent = `frame ${i + 1}/${replay.states.length} · ${seconds}s / ${total}s`;
...
document
	.getElementById("curate-refresh")
	.addEventListener("click", refreshCurate);
...
document
	.getElementById("curate-keep")
	.addEventListener("click", () => patchSelected({ review: "kept" }));
document
	.getElementById("curate-reject")
	.addEventListener("click", () => patchSelected({ review: "rejected" }));
document
	.getElementById("curate-unreview")
	.addEventListener("click", () => patchSelected({ review: "unreviewed" }));
...
document
	.getElementById("curate-trim-reset")
	.addEventListener("click", () => patchSelected({ trim: null }));
curateEl.raw.addEventListener("change", async () => {
	await safeStopPhysical();
	await loadCuratedReplay();
});
document
	.getElementById("curate-notes-save")
	.addEventListener("click", () =>
		patchSelected({ notes: curateEl.notes.value }),
	);
...
function selectedExportTasks() {
	return [...exportEl.tasks.querySelectorAll("input:checked")].map(
		(input) => input.value,
	);
}
...
function invalidateExportPreview() {
	...
	exportEl.summary.textContent =
		"Validate the updated selection before export.";
...
async function pollExport(exportId) {
	...
	if (status.state === "complete") {
		exportEl.summary.textContent = `Export complete · ${status.episodes} episodes · ${status.root}`;
		exportEl.start.disabled = false;
		return;
	}
	...
	setTimeout(
		() =>
			pollExport(exportId).catch((error) => {
				exportEl.summary.textContent = error.message;
				exportEl.summary.classList.add("err");
			}),
		1000,
...
// A restarted cockpit begins idle even when recovery has durable work.
refreshCollection()
	.then(refreshTasks)
	.then(() => renderCollection(collectionSnapshot))
	.catch((error) => {
		collectionEl.status.textContent = error.message;
		collectionEl.status.classList.add("err");
	});
